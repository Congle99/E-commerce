...
const ProductDetail = () => {
  ...
  const handleDeleteReview = (reviewId) => {
    ...
    http
      ...
      .catch(() => alert("Đánh giá không tồn tại!"));
  };

  const handleSubmitReview = async (e) => {
    e.preventDefault();
    if (!newComment.trim()) return alert("Vui lòng nhập nội dung đánh giá. Không được để trống");
    if (newComment.trim().length > 255) {
      return alert("Vui lòng nhập nội dung đánh giá dưới 255 ký tự.");
    }
    // const token = JSON.parse(localStorage.getItem("token"));
    const token = localStorage.getItem("token");
    if (!token) return alert("Vui lòng đăng nhập để gửi đánh giá.");
    // Parse token để lấy thông tin user ID nếu có trong token (giả sử bạn lưu user ID)
    const user = JSON.parse(localStorage.getItem("user")); // lưu `user` khi đăng nhập
